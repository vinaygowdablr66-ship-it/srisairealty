...
function createApp(express) {
  ...
  // Generates a signed URL so the browser can upload an image DIRECTLY to
  // Supabase Storage, bypassing the Vercel Function 4.5MB body limit.
  app.post('/api/uploads/sign', requireAuth, async (req, res) => {
    try {
      ...
      const allowed = /jpeg|jpg|png|gif|webp/;
      if (!allowed.test(path.extname(fileName).toLowerCase())) {
        return res.status(400).json({ error: 'Only image files are allowed' });
      }
      const signed = await db.createSignedUploadUrl(fileName);
      res.json(signed);
    } catch (err) {
      console.error('Create signed upload error:', err);
  ...
  app.post('/api/properties', requireAuth, upload.array('images', 8), async (req, res) => {
    try {
      ...
      let imageUrls = files.length ? await db.uploadImages(files) : [];
